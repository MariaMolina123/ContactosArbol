...
public class ArbolBinario {
    ...
    //Obteneer el nodo más a la derecha del subarbol izquierdo
    private Nodo obtenerNodoSucesorDere(Nodo n) {
        n = n.izquierdo;
    ...
    public void eliminarNodo(Nodo n) {
        ...
        if (apuntador.izquierdo.nombre.compareTo(n.izquierdo.nombre) < 0) {
    ...
    public void Buscar(String entradaUser, Nodo n) {
        Nodo apuntador = raiz;
        boolean encontrado = false;
        while (apuntador != null && !encontrado) {
            if (apuntador == n) {
                ...
                boolean result = entradaUser.contentEquals(n.nombre);
                if (result == true) {
